...
namespace ErrorHandlingApp.Controllers
{
    ...
    public class HomeController : Controller
    {
        ...
        {
            var memory = DownloadSingFile("Profile.txt", "wwwroot\\My Files");
            return File(memory.ToArray(), "text/plain", "Profile.txt");
        }

        private MemoryStream DownloadSingFile (string filename, string uploadPath)
        {
            var path = Path.Combine(Directory.GetCurrentDirectory(), uploadPath, filename);
            var memory = new MemoryStream();
            if (System.IO.File.Exists(path))
            {
                var net = new System.Net.Http.WebClient();
                var data = net.DownloadData(path);
                var content = new System.IO.MemoryStream((int)data);
                memory = content;
            }
            memory.Position = 0;
            return memory;
